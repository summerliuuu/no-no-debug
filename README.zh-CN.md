...
| 真实环境验证 | 修复是否用真实生产命令验证，而不是沙箱/简化测试 |
| 跨 agent 采信 | 另一个 agent 说"通过"时，是否独立复测而不是盲信 |
...
手动安装或从 v1.1.0 升级时，还需要把 hook 脚本复制过去：
...
然后把 `SKILL.md` → Mechanism 6 里的 `PostToolUseFailure` / `UserPromptSubmit` 配置块合并进 `~/.claude/settings.json`。

**前置依赖**：shipped 的 hook 用到 `python3`（UserPromptSubmit 过滤器）和 `jq`（PostToolUseFailure 解析器）。macOS 和主流 Linux 发行版默认都带；缺失时对应 hook 会优雅降级 —— `python3` 缺失时纠正检测静默跳过，`jq` 缺失时会在 log 里写一次 `jq_not_installed` 标记。想要完整覆盖可以 `brew install jq`（macOS）或用你系统的包管理器装一下。
